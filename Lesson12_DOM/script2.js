const users = [
  ...
  // 2. Дан массив с объектами (данные о пользователях). На основе этого массива создать карточки пользователей (имя, фамилия, возраст)
...
.forEach( el => {
  ...
  cardElem.innerText = el.online ? 'online' : 'offline';
  
  avatarElem.setAttribute('src', el.avatar);
  avatarElem.setAttribute('alt', el.avatar);

  //avatarElem.src = el.avatar;
  //avatarElem.alt = `${el.firstname} ${el.lastname}`;

  cardElem.append(avatarElem, firstnameElem, lastnameElem, ageElem);
